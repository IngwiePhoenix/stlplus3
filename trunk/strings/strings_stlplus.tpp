...
namespace stlplus
{
                                ...
                                const std::string& separator)
  {
    ...
    result += "nodes:";
    result += separator;
    for (TYPENAME digraph<NT,AT>::const_iterator n = values.begin(); n != values.end(); ++n)
    {
      if (n != values.begin()) result += separator;
      result += node_to_string_fn(*n);
    }
    result += "arcs:";
    result += separator;
    for (TYPENAME digraph<NT,AT>::const_arc_iterator a = values.arc_begin(); a != values.arc_end(); ++a)
    {
      if (a != values.arc_begin()) result += separator;
      result += arc_to_string_fn(*a);
    }
    return result;
  }
  ...
  {
    std::string result;
    for (TYPENAME hash<K,T,H,E>::const_iterator i = values.begin(); i != values.end(); i++)
    {
